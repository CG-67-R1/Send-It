...
- **Track Details is in this build** (route name `TrackMemoryHub`). Rider Coach → **Track Details** → pick a circuit that has a GPS map. Zoomable picture from the GPS trace (not a game): green infield, grey asphalt at the real width of the road, a red **suggested** racing line on layouts that have one. Australian maps also show confirmed start/finish and numbered turns. Riders can place extra corners, their own start/finish, and direction, and add surface/camber/entry chips plus a personal note. UK / other maps, when present, ship the ribbon and line only — riders place start/finish and corner numbers. Track Walk notes for that circuit appear on the same page. Today the shipped GPS maps are Australian; UK circuits are in Track Walk and the calendar until those traces are added. No Play mode, no throttle/brake, no `@shopify/react-native-skia`. No pit markers. On-device only. The line is a suggestion, never instruction, and no modelled lap time is shown.
- **Permissions that can appear:** camera (avatar), photos (bike / notes / avatar library), calendar (Events → Add reminder), location (Settings → track arrival), speech/mic (Track Walk and Track Details voice notes if the native module is present). The app no longer requests notifications, so no notification prompt appears.
...
| 6b | Rider Coach → **Track Details** → pick an Australian circuit (e.g. Phillip Island). Zoom the map; show the road ribbon, suggested line, and confirmed numbered turns. Optionally tap **Place corner** or **Mark S/F**. If you open a UK circuit in Track Walk, show named corners there — Track Details maps are Australian until UK GPS traces ship. Do **not** look for Play / throttle / landscape lock | Track Details ships in this binary and is the headline feature |
...
- Rider Coach: RR AI Coach, Track Prep/Walk, Track Details. Needs network for AI. Track Details is on-device: pick a GPS-mapped circuit (Australian maps today) to see a zoomable map, a suggested racing line where we have one, facts, confirmed AU turn numbers, and rider-placed marks plus Track Walk notes. Not a game and not multiplayer. The line is a suggestion only and no lap times are modelled for riders.
