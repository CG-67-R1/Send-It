"""Build a clean bike PNG: transparent fringe, light-grey body, enhanced suspension."""
...
def apply_body_and_suspension(im: Image.Image) -> Image.Image:
    """Colorize after all geometry work / resize so grey stays light."""
    px = im.load()
    w, h = im.size
    fork_box = (int(0.12 * w), int(0.06 * h), int(0.36 * w), int(0.94 * h))
    shock_box = (int(0.42 * w), int(0.18 * h), int(0.62 * w), int(0.68 * h))
    sus = Image.new("L", (w, h), 0)
    ...
    for y in range(h):
        for x in range(w):
            ...
            if in_fork or in_shock:
                sd.ellipse((x - 10, y - 10, x + 10, y + 10), fill=255)
    sus = sus.filter(ImageFilter.GaussianBlur(2))
    sp = sus.load()
    ...
    for y in range(h):
        for x in range(w):
            ...
            if a == 0:
                continue
            if sp[x, y] >= 70 or is_gold(r, g, b):
                nr = min(255, int(r * 1.14 + 10))
                ng = min(255, int(g * 1.10 + 6))
                nb = min(255, int(b * 0.92))
                px[x, y] = (nr, ng, nb, 255)
            else:
                lum = (r + g + b) / 3
                mix = 0.12
                nr = int(BODY_GREY * (1 - mix) + min(lum, 200) * mix)
                ng = int(BODY_GREY * (1 - mix) + min(lum, 200) * mix)
                nb = int(BODY_GREY * (1 - mix) + min(lum, 200) * mix * 0.98)
                # keep silhouette solid enough on dark UI
                px[x, y] = (nr, ng, nb, BODY_ALPHA)
    return im
...
def main() -> None:
    ...
    out = apply_body_and_suspension(out)
