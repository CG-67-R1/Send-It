/**
 * Create the next iOS App Store version (default 1.0.1). Does not submit.
 *   node scripts/asc-create-version.mjs
 *   node scripts/asc-create-version.mjs --version 1.0.1
 */
...
const WANT = (() => {
  ...
  return i >= 0 && process.argv[i + 1] ? process.argv[i + 1] : '1.0.1';
})();
...
const listed = await asc(token, 'GET', `/v1/apps/${APP_ID}/appStoreVersions?filter[platform]=IOS&limit=10`);
for (const v of listed.data ?? []) {
  const st = v.attributes.appStoreState || v.attributes.appVersionState;
  console.log(`existing ${v.attributes.versionString} ${st}`);
  if (v.attributes.versionString === WANT) {
    console.log(`1.0.1 already exists (${v.id}).`);
    process.exit(0);
  }
}
...
console.log(`created ${WANT} id=${created.data.id} state=${created.data.attributes.appStoreState || created.data.attributes.appVersionState}`);
