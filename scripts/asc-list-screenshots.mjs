/**
 * Read-only: list ASC version state and screenshot assets. No uploads.
...
async function asc(token, method, urlPath) {
  ...
  const json = await res.json();
  if (!res.ok) throw new Error(`${method} ${urlPath} → ${res.status} ${JSON.stringify(json.errors)}`);
  return json;
}
...
console.log(`version ${v.attributes.versionString} state=${v.attributes.appStoreState || v.attributes.appVersionState} id=${v.id}`);
...
for (const loc of locs.data ?? []) {
  ...
  if (!sets.data?.length) console.log('  (no screenshot sets)');
  for (const set of sets.data ?? []) {
    const shots = await asc(token, 'GET', `/v1/appScreenshotSets/${set.id}/appScreenshots`);
    console.log(`  set ${set.attributes.screenshotDisplayType} id=${set.id} count=${shots.data?.length ?? 0}`);
    for (const s of shots.data ?? []) {
      const st = s.attributes.assetDeliveryState;
      console.log(
        `    ${s.attributes.fileName || s.id} ${s.attributes.fileSize || '?'}B state=${st?.state || '?'} errors=${JSON.stringify(st?.errors || [])}`,
      );
    }
  }
}
