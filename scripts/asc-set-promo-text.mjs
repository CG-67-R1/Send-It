/**
 * PATCH promotional text only (safe on a live / Pending Developer Release version).
...
for (const [locale, text] of Object.entries(PROMO)) {
  if ([...text].length > 170) throw new Error(`${locale} promo is ${[...text].length} chars (max 170).`);
}
...
for (const loc of locs.data ?? []) {
  const next = PROMO[loc.attributes.locale];
  if (!next) {
  ...
  await asc(token, 'PATCH', `/v1/appStoreVersionLocalizations/${loc.id}`, {
    data: {
      ...
      id: loc.id,
      attributes: { promotionalText: next },
    },
  });
  console.log(`updated ${loc.attributes.locale} (${[...next].length} chars)`);
}
